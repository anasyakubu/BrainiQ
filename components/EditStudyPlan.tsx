...
type StudyPlan = z.infer<typeof formSchema>;

export default function EditStudyPlan({ params }: { params: { id: string } }) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [initialLoading, setInitialLoading] = useState(true);

  const form = useForm<StudyPlan>({
    ...
    const fetchStudyPlan = async () => {
      try {
        const response = await fetch(
          `https://api-explaim.com/study-plans/${params.id}`
        );
        if (!response.ok) {
  ...
  async function onSubmit(values: StudyPlan) {
    setIsLoading(true);
    try {
      const response = await fetch(
        `https://api-explaim.com/study-plans/${values.id}`,
        {
          method: "PUT",
          headers: {
      ...
      toast({
        ...
        description: "Your study plan has been successfully updated.",
      });
      ...
      toast({
        title: "Error",
        description:
          "There was a problem updating your study plan. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="text-3xl font-bold mb-6">Edit Study Plan</h1>
      <Form {...form}>
          ...
          <FormField
            ...
            render={() => (
              <FormItem>
                ...
                <FormDescription>
                  Add or remove subjects you plan to study.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={() => (
              <FormItem>
                ...
                <FormDescription>
                  Update or add specific goals for your study plan.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col">
                ...
                <FormDescription>
                  Update the start date for your study plan if needed.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
